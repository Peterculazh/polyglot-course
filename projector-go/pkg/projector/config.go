...
func getArgs(opts *Opts) ([]string, error) {
	...
	if operation == Add {
		if len(opts.Args) != 3 {
			return nil, fmt.Errorf("add requires 2 arguments but got %v", len(opts.Args)-1)
		}
	...
	if operation == Remove {
		if len(opts.Args) != 2 {
			return nil, fmt.Errorf("remove requires 1 arguments but got %v", len(opts.Args)-1)
		}
		return opts.Args[1:], nil
	}

	if len(opts.Args) > 1 {
		return nil, fmt.Errorf("print requires requires 0 or 1 argument/s but got %v", len(opts.Args))
	}
